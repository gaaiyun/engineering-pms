...
- 2026-09-02 已完成删除影响预检兼容修复：生产历史 `tasks` 表缺少 `approved_by` 时，Hook 会按实际 schema 跳过不存在字段，不会阻断人员重配；生产业务库未写入或删除。
...
| 项目 | 结果 |
|---|---|
| 部署 | Web/MCP 基线提交 `c4c298f`（2026-08-20 06:45 CST）；人员维护与 Hook 兼容修复提交 `7bf595f`；人员查重大小写修复提交 `0bd61b4`（2026-09-02） |
