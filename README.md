...
- 2026-09-02 已完成人员权限与会话一致性收口：`people_manage` 只能显式授予，部门字段统一为七项受控单选，Agent 调整角色会撤销旧会话，不存在账号的删除预检返回明确 `404`；生产业务记录未增删。
...
- 生产当前仍保留既有测试业务数据（本次验证未删除）：线上只读核对为 12 个启用账号、4 个项目、9 个任务。交付负责人可按“停用/改名/引用保护”流程逐项重配，不能把有历史责任的账号硬删。
- MCP sidecar 已通过生产 HTTPS 提供 24 个受控业务工具；其中 5 个“人员维护”工具仅对全项目管理员服务账号开放，不提供 AI、服务器或 PocketBase 超级管理员能力。
...
| 项目 | 结果 |
|---|---|
| 部署 | Web/MCP 基线提交 `c4c298f`（2026-08-20 06:45 CST）；人员维护与 Hook 兼容修复提交 `7bf595f`；人员查重大小写修复提交 `0bd61b4`；人员权限与会话一致性提交 `c7daa05`（2026-09-02 19:02 CST） |
| 生产备份 | 精确路径记录于私有运维接力文档 |
| Web | `<production-web-url>` |
| MCP | `<production-web-url>/mcp`；无认证 MCP POST 请求返回 401 |
| 自动化 | 后端既有基线 `64/64`、最终差异 `15/15`；人员维护隔离 QA `47/47`；历史 schema 兼容 QA 通过；前端 `204/204`；Playwright `30/30`；MCP `14/14` |
| Realtime | 生产 SSE 返回 200 |
| APK | `3.06` / `46`；SHA-256 `6FAD6BD746DA4FEFC9F15C955497EFDC4DE5F83721A10915A282343E442B4044`；静态验收通过，真机待验收 |
