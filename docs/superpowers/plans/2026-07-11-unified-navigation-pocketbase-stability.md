...
- [x] 先为角色可见性、旧路由映射和 surface 决策写失败测试。
- [x] 定义 `AppRole`、`RouteId`、`NavItem`、`AppSurface` 与无 DOM 的解析函数。
- [x] 测试 employee、manager、admin 与 native/coarse/fine pointer 组合。

## Task 3：唯一 AppShell

- [x] 让 `AppShell` 独占桌面侧栏、折叠侧栏和移动底栏。
- [x] 删除 `Home.tsx` 内部 PC 侧栏、独立断点和底栏。
- [x] 删除 `AdminDashboard.tsx` 无条件底栏，把页面 Tab 降为内容区二级导航。
- [x] 保证任一 viewport 只渲染一种主导航。

## Task 4：角色路由和兼容跳转

- [x] 所有角色登录进入 `/app`。
- [x] 增加 `/me`、`/system/users`、`/system/ai`、`/system/import` 与 admin-only guard。
- [x] 把 `/manager` 和旧 `/admin?tab=` 映射到新路由。
- [x] 保留项目详情、时间轴、任务和通知深链接。

## Task 5：PocketBase 恢复资产

- [x] 提交维护脚本、systemd 模板和 Nginx `/pb` 片段。
...
- [x] Vitest 全量、changed-files ESLint、生产构建全部通过。
- [x] Playwright 覆盖手机、触控平板、键鼠窗口、桌面和三个角色。
- [x] 使用 G 盘 Gradle 缓存构建 debug APK。
