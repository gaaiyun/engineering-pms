...
MCP 只通过受控业务 API 操作项目、任务、评论、交接和查询；不能直接写 SQLite，不能修改用户角色、AI 配置、服务器设置或账号密码。生产 `tools/list` 返回 20 个业务工具，不包含账号或系统管理工具。归档、删除、批量改派和交接审批需要先预览，再使用五分钟一次性确认码执行；所有写入都要求幂等键。
...
3.06 生产验收：MCP 测试 `11/11`，真实 `initialize`、`tools/list` 和受控请求通过；无认证 MCP POST 请求返回 401。
