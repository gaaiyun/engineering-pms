...
排行榜在当前版本中**只是一个前端展示的静态示例**，方便你先体验 UI：

- 后续如果你真的需要“按月统计、自动排名”等功能，再单独设计 `rankings` 集合和定时任务。
- 现在可以不在 PocketBase 里建这个表，避免增加复杂度。
...
> 说明：`tasks.score` 与 `tasks.approved` 不在当前 api.ts 的 Task 接口中。排行榜积分来自 `users.flower_count`；任务“审核通过”通过 handoffs 的 `status=approved` 与 `approved_task` 体现。
